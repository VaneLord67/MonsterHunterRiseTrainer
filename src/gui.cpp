...
ImGuiWindowFlags main_window_flags = ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoSavedSettings |
...
bool GuiMain(const std::function<void()>& customUI) {
    ...
    {
        std::cerr << "Failed to create device D3D" << std::endl;
        CleanupDeviceD3D();
    ...
    // load font from file
    io.Fonts->AddFontFromFileTTF("c:\\Windows\\Fonts\\msyh.ttc", 25.0f, nullptr, io.Fonts->GetGlyphRangesChineseFull());
